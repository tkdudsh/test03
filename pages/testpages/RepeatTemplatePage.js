...
export default function RepeatTemplatePage({ sentence, nextScreen, navigation }) {
  ...
  const startRecording = async () => {
    try {
      ...
      setIsRecording(true); // ✅ 녹음 시작 상태 업데이트
...
const styles = StyleSheet.create({
  ...
  taskContainer: { backgroundColor: '#fff', borderRadius: 10, padding: 20, elevation: 2 },
  taskText: { fontSize: 18, marginBottom: 20, color: '#333' },
  recordButton: { backgroundColor: '#4A90E2', paddingVertical: 12, borderRadius: 8, alignItems: 'center' },
  buttonText: { fontSize: 16, color: '#fff', fontWeight: 'bold' },
  uriText: { fontSize: 14, color: 'green', marginTop: 10, textAlign: 'center' },
